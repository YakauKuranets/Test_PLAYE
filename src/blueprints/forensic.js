...
<html lang="ru">
  ...
  <body>
    ...
    <div class="meta">
      ...
      <p><strong>SHA‑256 подпись отчёта:</strong> ${escapeHtml(payload.signature || "—")}</p>
    </div>
    ...
    elements.exportLogButton.addEventListener("click", async () => {
      // Формируем payload журнала и вычисляем подпись SHA‑256.
      const payload = {
        caseId: elements.caseId.value.trim(),
        owner: elements.caseOwner.value.trim(),
        status: elements.caseStatus.value,
      ...
      try {
        payload.signature = await hashString(JSON.stringify(payload.entries));
      } catch (err) {
        console.warn('Не удалось вычислить подпись для журнала', err);
      }
    ...
    elements.exportReportButton.addEventListener("click", async () => {
      ...
      // Добавляем подпись для отчёта по всему payload (включая хеши файлов и логи)
      try {
        payload.signature = await hashString(JSON.stringify(payload));
      } catch (err) {
        console.warn('Не удалось вычислить подпись отчёта', err);
      }
    ...
    elements.previewReportButton.addEventListener("click", async () => {
      const payload = buildReportPayload();
      try {
        payload.signature = await hashString(JSON.stringify(payload));
      } catch (err) {
        console.warn('Не удалось вычислить подпись отчёта для предпросмотра', err);
      }
